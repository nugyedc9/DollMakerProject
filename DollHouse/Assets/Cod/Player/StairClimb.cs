...
public class StairClimb : MonoBehaviour
{
    ...
    Rigidbody rigidBody;
     ...
     void stepClimb()
     {
         ...
         {
            Debug.DrawRay(stepRayLower.transform.position, transform.TransformDirection(Vector3.forward) * 0.3f, Color.red);
            RaycastHit hitUpper;
             if (!Physics.Raycast(stepRayUpper.transform.position, transform.TransformDirection(Vector3.forward), out hitUpper, 0.5f))
             {
                Debug.DrawRay(stepRayUpper.transform.position, transform.TransformDirection(Vector3.forward) * 0.5f, Color.green);
